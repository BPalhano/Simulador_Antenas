...
PN = (2.5118864315 * 10 ** (-15))  # linear scale
sigLN = 8  # dB
...
def antenas(num, dist):  # Gerador de posicionamento de ERB's
    ...
    while (contador != num):
        ...
        arr1 = np.array(
            [dist * math.cos((2 * math.pi / num) * contador), dist * math.sin((2 * math.pi / num) * contador)])
...
def Fast_fadding(vet1):
    ...
    for i in range(len(vet1)):
        rv = rice.rvs(float(1 / np.sqrt(2)) , 0 )
...
for i in loop:  # Simulação
    ...
    D = Fast_fadding(P)  # Armazenamento do Ruído de Desvanecimento Rápido do estado atual.
